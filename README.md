...
Le loader attend uniquement le logo et les visuels du premier écran. Les images situées plus bas conservent le chargement différé natif de Next.js et ne bloquent plus l’accès à l’accueil. Sa durée minimale est de 200 ms et son délai de sécurité de 2,5 secondes. Le logo transparent utilisé par le loader est passé de 535 Ko en PNG à 119 Ko en WebP sans perte ; les bases existantes migrent automatiquement vers cette version.
...
Après l’optimisation du loader du 23 septembre 2026 : **27 tests serveur**, TypeScript, ESLint et le build de production réussissent. Six scénarios navigateur ciblés réussissent sur ordinateur et mobile pour le loader, les images publiques, la navigation, les débordements et l’accessibilité de l’accueil. Les vérifications précédentes couvrent aussi les sept langues, le panneau d’administration et la migration des guides.
...
- **Logo actuel** : le fichier `ChatGPT Image 10 sept. 2026, 04_19_39.png`, fourni et demandé comme nouvelle identité par LEA, remplace le logo de la brochure. Une version transparente WebP de 640 px, optimisée pour l’écran et le loader, est enregistrée dans `public/images/lea-logo-rose.webp`. Le composant partagé `Brand` réserve une boîte et utilise `object-fit: contain` pour afficher le symbole et les lettres LEA sans étirement dans le site et le panneau ; la mention « Aesthetic » reste du texte HTML. L’en-tête, le pied de page, la connexion, le tableau de bord et l’image de partage l’utilisent. Le logo reste remplaçable dans la médiathèque. Le précédent fichier `lea-logo.png` reste disponible pour les versions archivées. Un fichier vectoriel officiel reste souhaitable.
